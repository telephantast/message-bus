 ...
 * @internal
 */
final readonly class Recoverability
{
    public function __construct(
        private RecoverabilityPolicy $policy,
        private Dispatcher $dispatcher,
        private DeadLetterStorage $deadLetterStore,
     ...
     */
    public function process(string $endpoint, InboundEnvelope $envelope, callable $operation): Disposition
    {
        ...
        while (true) {
            try {
                ...
                try {
                    ...
                    return Disposition::Ack;
                } catch (\Throwable $recoverabilityError) {
                    $this->logger->error('Recoverability failed; requeueing message.', $logContext + [
                        'exception' => $recoverabilityError,
                        'handling_exception_class' => $error::class,
                        'immediate_retry_count' => $immediateRetryCount,
                        'delayed_retry_count' => $delayedRetryCount,
                    ]);
